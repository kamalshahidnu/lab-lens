...
class IntelligentBiasHandler:
    ...
    def __init__(self, input_path: str = "data/processed", output_path: str = "logs", config: Dict = None):
        ...
        if config and "bias_detection_config" in config:
            ...
            self.mitigation_config = {"auto_mitigation": True, "mitigation_enabled": True}
    ...
    def load_bias_report(self, filename: str = "bias_report.json") -> Dict[str, Any]:
        ...
        if not os.path.exists(filepath):
            raise FileNotFoundError(
                f"Bias report not found at {filepath}. " f"Please run bias_detection.py first to generate the report."
            )
        ...
        if missing_sections:
            logger.warning(f"Bias report missing sections: {missing_sections}. " f"Mitigation may be limited.")
    ...
    def determine_mitigation_strategy(self, bias_report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        strategy = {"action": "none", "methods": [], "reasoning": "", "requires_review": False, "demographic_targets": []}
        ...
        for demo_col, demo_stats in residual_analysis.items():
            ...
            if interpretation == "LEGITIMATE_VARIATION":
                ...
                logger.info(
                    f"{demo_col}: Residual CV {residual_cv:.2f}% indicates legitimate variation. " f"No mitigation required."
                )
                continue
            ...
            if residual_cv < self.thresholds["residual_cv_threshold"]:
                ...
                logger.warning(
                    f"{demo_col}: Residual CV {residual_cv:.2f}% indicates moderate bias. " f"Applying feature normalization."
                )
                strategy["action"] = "moderate"
                strategy["methods"].append("feature_normalization")
                strategy["demographic_targets"].append(demo_col)
                strategy["reasoning"] += f"Moderate unexplained bias detected for {demo_col} (CV: {residual_cv:.1f}%). "
        ...
        for demo_col, quality_metrics in quality_analysis.items():
            for metric_name, metric_data in quality_metrics.items():
                if metric_data.get("assessment") == "DISPARITY":
                    logger.warning(
                        f"Quality disparity detected for {demo_col}: {metric_name}. " f"Adding quality-focused mitigation."
                    )
                    if "quality_monitoring" not in strategy["methods"]:
                        strategy["methods"].append("quality_monitoring")
                    strategy["reasoning"] += f"Quality disparity in {metric_name} for {demo_col}. "
    ...
    def normalize_features_by_demographics(self, df: pd.DataFrame, demo_cols: List[str]) -> pd.DataFrame:
        """
        ...
        for feature in available_features:
            for demo_col in demo_cols:
                ...
                original_cv = (df.groupby(demo_col)[feature].mean().std() / df.groupby(demo_col)[feature].mean().mean()) * 100
                normalized_cv = (
                    df_normalized.groupby(demo_col)[feature].mean().std()
                    / df_normalized.groupby(demo_col)[feature].mean().mean()
                ) * 100

                logger.info(
                    f"Normalized {feature} by {demo_col}: " f"CV reduced from {original_cv:.2f}% to {normalized_cv:.2f}%"
                )
    ...
    def add_quality_monitoring_flags(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for flag_name, section_col in critical_sections.items():
            if section_col in df.columns:
                ...
                df_flagged[flag_name] = ((~df[section_col].isna()) & (df[section_col].astype(str).str.len() > 10)).astype(int)
    ...
    def apply_mitigation(self, df: pd.DataFrame, strategy: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        mitigation_details = {"methods_applied": [], "changes_made": [], "features_modified": []}
        ...
        if "feature_normalization" in strategy["methods"]:
            ...
            # Normalize features
            mitigated_df = self.normalize_features_by_demographics(mitigated_df, demo_targets)

            mitigation_details["methods_applied"].append("feature_normalization")
            mitigation_details["changes_made"].append(f"Normalized features across {', '.join(demo_targets)}")
            mitigation_details["features_modified"].extend(["text_chars", "text_tokens", "word_count"])
        ...
        if "quality_monitoring" in strategy["methods"]:
            ...
            mitigation_details["changes_made"].append("Added quality monitoring flags for critical sections")
    ...
    def calculate_post_mitigation_metrics(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        for demo_col, metric_name in [("gender", "gender_cv"), ("ethnicity_clean", "ethnicity_cv"), ("age_group", "age_cv")]:
            if demo_col not in df.columns:
                continue
    ...
    def generate_mitigation_report(
        ...
        if mitigation_applied:
            effectiveness = self._assess_mitigation_effectiveness(original_metrics, post_metrics, strategy)
            report["effectiveness_assessment"] = effectiveness

        # Generate recommendations based on results
        report["recommendations"] = self._generate_recommendations(
            bias_report, strategy, mitigation_applied, report.get("effectiveness_assessment", {})
        )

        return report

    def _assess_mitigation_effectiveness(
        self, original_metrics: Dict[str, float], post_metrics: Dict[str, float], strategy: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_recommendations(
        self, bias_report: Dict[str, Any], strategy: Dict[str, Any], mitigation_applied: bool, effectiveness: Dict[str, Any]
    ) -> List[str]:
        """
        ...
        if not mitigation_applied:
            ...
            recommendations.append("Continue regular monitoring to ensure variation remains appropriate.")
            recommendations.append("Ensure ML models are trained to handle clinical complexity fairly.")
        ...
        if strategy.get("requires_review", False):
            ...
            recommendations.append("Conduct detailed audit of documentation procedures by demographic group.")

        # Recommendations based on effectiveness
        if effectiveness.get("overall_effectiveness") == "effective":
            recommendations.append("Mitigation was successful. Use mitigated dataset for ML model training.")
            recommendations.append("Continue monitoring bias metrics with regular re-assessment.")
            ...
            recommendations.append("Review mitigation parameters and consider more aggressive normalization.")
        else:
            recommendations.append(
            ...
            recommendations.append(
                "Focus on fairness constraints during model training rather than " "additional data preprocessing."
            )

        # Quality-related recommendations
        quality_analysis = bias_report.get("quality_parity_analysis", {})
        has_quality_issues = any(
            any(m.get("assessment") == "DISPARITY" for m in metrics.values()) for metrics in quality_analysis.values()
        )
    ...
    def run_mitigation_pipeline(self) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        mitigation_report = self.generate_mitigation_report(
            bias_report, original_metrics, post_metrics, strategy, mitigation_details, mitigation_applied
        )
...
if __name__ == "__main__":
    ...
    handler = IntelligentBiasHandler(
        input_path=config["pipeline_config"]["output_path"], output_path=config["pipeline_config"]["logs_path"], config=config
    )
