...
class MIMICBiasDetector:
    ...
    def _analyze_metric_by_demographics(self, df: pd.DataFrame, metric: str, agg_funcs: List[str] = None) -> Dict[str, Any]:
        """
    ...
    def _perform_statistical_test(self, df: pd.DataFrame, metric: str, demo_col: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return {
                ...
                "interpretation": (
                    ...
                    if p_value < self.thresholds["significance_threshold"]
    ...
    def detect_documentation_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        bias_report = {"documentation_length_bias": {}, "documentation_quality_bias": {}, "statistical_tests": {}}
    ...
    def detect_clinical_risk_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for metric in risk_metrics:
            if metric in df.columns:
                risk_bias_report[metric] = self._analyze_metric_by_demographics(df, metric, ["mean", "median", "std", "count"])
    ...
    def detect_treatment_complexity_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for metric in treatment_metrics:
            if metric in df.columns:
                treatment_bias[metric] = self._analyze_metric_by_demographics(df, metric, ["mean", "median", "count"])
    ...
    def detect_lab_testing_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for metric in lab_metrics:
            if metric in df.columns:
                lab_bias_report[metric] = self._analyze_metric_by_demographics(df, metric, ["mean", "median", "std", "count"])
    ...
    def detect_readability_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for metric in readability_metrics:
            if metric in df.columns:
                readability_bias[metric] = self._analyze_metric_by_demographics(df, metric, ["mean", "median", "count"])
    ...
    def detect_adjusted_bias(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        *** THIS IS THE MOST CRITICAL ANALYSIS ***
        ...
        """
        adjusted_bias_report = {"analysis_performed": False, "reason_skipped": None}
    ...
    def _interpret_adjusted_results(self, residual_stats: Dict, r2_score: float) -> str:
        ...
        biased_demos = [demo for demo, stats in residual_stats.items() if stats.get("interpretation") == "POTENTIAL_BIAS"]
    ...
    def create_bias_visualizations(self, df: pd.DataFrame):
        ...
        if "age_group" in df.columns and "text_chars" in df.columns:
            ...
            if hasattr(df["age_group"], "cat"):
                ...
                labels = [f"{age}\n(n={age_counts.get(age, 0)})" for age in age_order if age in unique_ages]
        ...
        if "treatment_intensity" in df.columns and "age_group" in df.columns:
            ...
            ax.set_title("Treatment Intensity by Age Group\n(Clinical complexity indicator)", fontsize=12, fontweight="bold")
        ...
        if "documentation_completeness" in df.columns and "ethnicity_clean" in df.columns:
            ...
            ax.set_title(
                "Documentation Completeness by Ethnicity\n(Quality parity assessment)", fontsize=12, fontweight="bold"
            )
    ...
    def create_bias_summary(self, report: Dict) -> pd.DataFrame:
        ...
        """
        summary_data = {"Analysis_Stage": [], "Metric": [], "Value": [], "Alert": [], "Interpretation": []}
