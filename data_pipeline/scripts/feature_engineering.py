...
def find_repo_root(start: Path = Path.cwd()) -> Path:
    """
    Find the repository root by looking for data_pipeline directory

    This walks up the directory tree until it finds a directory containing
    either 'data_pipeline/' or 'configs/' subdirectory.
    ...
    while cur != cur.parent:
        if (cur / "data_pipeline").exists() or (cur / "configs").exists():
            return cur
        cur = cur.parent
    return start
...
# ========================================================================
MED_SUFFIXES = ["pril", "sartan", "olol", "dipine", "statin", "azole", "tidine", "caine", "cycline", "cillin", "mycin", "sone"]
...
# ========================================================================
ACUTE_TERMS = ["acute", "sudden", "rapid", "urgent", "emergency", "immediate", "new onset"]
...
# ========================================================================
CHRONIC_TERMS = ["chronic", "longstanding", "history of", "ongoing", "persistent", "long term"]
...
def calculate_readability_scores(text: str) -> Dict[str, float]:
    ...
    """
    if not isinstance(text, str) or not text:
        return {"flesch_reading_ease": 0, "avg_syllables_per_word": 0, "vocabulary_richness": 0}
...
def calculate_missingness_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    section_cols = ["discharge_diagnosis", "discharge_medications", "follow_up", "chief_complaint", "hospital_course"]
    ...
    if existing_sections:
        ...
        missing_count = sum(df[col].isna() | (df[col] == "") for col in existing_sections)

        df["missing_section_count"] = missing_count.astype("Int64")

        # Calculate completeness score (0-1)
        # 1.0 = all sections present, 0.0 = no sections present
        df["documentation_completeness"] = ((len(existing_sections) - missing_count) / len(existing_sections)).clip(0, 1)
    else:
...
def calculate_clinical_risk_features(df: pd.DataFrame, text_col: str) -> pd.DataFrame:
    ...
    df["high_risk_score"] = df[text_col].map(lambda s: count_terms(s, HIGH_RISK_TERMS)).astype("Int64")

    # ---- Positive Outcome Term Count ----
    # Counts: improved, stable, recovered, etc.
    df["positive_outcome_score"] = df[text_col].map(lambda s: count_terms(s, POSITIVE_OUTCOME_TERMS)).astype("Int64")

    # ---- Risk to Outcome Ratio ----
    # Add 1 to denominator to avoid division by zero
    # Clip at 10.0 to prevent extreme outliers
    df["risk_outcome_ratio"] = (df["high_risk_score"] / (df["positive_outcome_score"] + 1)).clip(upper=10.0)

    # ---- Acute Presentation Score ----
    # Counts: acute, sudden, emergency, etc.
    df["acute_presentation_score"] = df[text_col].map(lambda s: count_terms(s, ACUTE_TERMS)).astype("Int64")

    # ---- Chronic Condition Score ----
    # Counts: chronic, longstanding, history of, etc.
    df["chronic_condition_score"] = df[text_col].map(lambda s: count_terms(s, CHRONIC_TERMS)).astype("Int64")

    # ---- Acute to Chronic Ratio ----
    # Add 1 to denominator to avoid division by zero
    # Clip at 5.0 to prevent extreme outliers
    df["acute_chronic_ratio"] = (df["acute_presentation_score"] / (df["chronic_condition_score"] + 1)).clip(upper=5.0)
...
def calculate_medical_density(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["disease_density"] = (df["kw_chronic_disease"] / denom).fillna(0).clip(upper=5.0)

    # ---- Medication Density ----
    # Medication mentions per sentence
    df["medication_density"] = (df["kw_medications"] / denom).fillna(0).clip(upper=5.0)

    # ---- Symptom Density ----
    # Symptom mentions per sentence
    df["symptom_density"] = (df["kw_symptoms"] / denom).fillna(0).clip(upper=5.0)
...
def calculate_treatment_complexity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["treatment_intensity"] = (med_score + lab_score + comorbid_score).fillna(0).clip(upper=20.0)
...
def add_section_flags(df: pd.DataFrame, text_col: str = "cleaned_text") -> pd.DataFrame:
    ...
    for flag, patterns in SECTION_PATTERNS.items():
        df[flag] = tl.map(lambda s: int(any(re.search(p, s) is not None for p in patterns))).astype("int8")
...
def engineer_features(df: pd.DataFrame, logger: logging.Logger, with_sections: bool = False) -> pd.DataFrame:
    ...
    optional = [
        c
        for c in ["has_allergies_section", "has_medications_section", "has_brief_hospital_course", "negation_density"]
        if c in df.columns
    ]
...
# ============================================================================
# This runs when you execute: python data_pipeline/scripts/feature_engineering.py
#
# What happens:
...
def main() -> None:
    ...
    # ====================================================================
    config_path = repo / "data_pipeline" / "configs" / "pipeline_config.json"
    ...
    if config_path.exists():
        ...
        default_input = repo / "data_pipeline" / "data" / "processed" / "processed_discharge_summaries.csv"
        default_output = repo / "data_pipeline" / "data" / "processed" / "mimic_features.csv"
        default_log = repo / "data_pipeline" / "logs" / "feature_engineering.log"
