...
class LabLensPipeline:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.scripts_dir = self.project_root / "data_pipeline" / "scripts"

        # Load configuration
        if config_path:
            self.config_path = Path(config_path)
        else:
            self.config_path = self.project_root / "data_pipeline" / "configs" / "pipeline_config.json"
    ...
    def _find_project_root(self) -> Path:
        """
        Find project root directory by looking for data_pipeline folder
        ...
        # Go up directory tree looking for data_pipeline
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        """
        return {
            "input_path": "data_pipeline/data/raw",
            "output_path": "data_pipeline/data/processed",
            "logs_path": "data_pipeline/logs",
    ...
    def _run_script(self, script_name: str, step_name: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, str(script_path)], cwd=str(self.project_root), capture_output=True, text=True, check=True
            )
    ...
    def _generate_results(self, success: bool) -> Dict[str, Any]:
        ...
        results = {
            ...
            "overall_status": {
                ...
                "data_quality": (
                    "excellent"
                    if self.pipeline_state["validation_score"] >= 90
                    else "good" if self.pipeline_state["validation_score"] >= 80 else "needs_improvement"
                ),
                "bias_status": (
                    "acceptable"
                    if self.pipeline_state["bias_score_after"] <= 10
                    else "moderate" if self.pipeline_state["bias_score_after"] <= 15 else "high"
                ),
...
Examples:
 ...
 python main_pipeline.py --skip-bias-detection --skip-bias-handling
    ...
    parser.add_argument("--skip-bias-handling", action="store_true", help="Skip automated bias handling step")
    ...
    try:
        ...
        print(
            f"Steps Completed: {len(exec_info['steps_completed'])}/{len(exec_info['steps_completed']) + len(exec_info['steps_failed'])}"
        )
