...
class MIMICPreprocessor:
    ...
    def calculate_text_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["avg_word_length"] = (
            df["cleaned_text"].fillna("").apply(lambda x: np.mean([len(word) for word in x.split()]) if x else 0)
        )

        # Count how many times medical terms appear (diagnosis, medication, etc.)
        # Higher count = more detailed medical documentation
        df["medical_term_count"] = (
            df["cleaned_text"]
            .fillna("")
            .str.count(r"\b(?:diagnosis|medication|symptom|procedure|laboratory|imaging)\b", flags=re.IGNORECASE)
        )
    ...
    def create_summary_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["completeness_score"] = df[section_cols].apply(
            lambda row: sum([1 for val in row if len(str(val)) > 10]) / len(section_cols), axis=1
        )
        ...
        df["complexity_score"] = (
            df["text_length"] / df["text_length"].max() * 0.5 + df["medical_term_count"] / df["medical_term_count"].max() * 0.5
        )
...
if __name__ == "__main__":
    ...
    preprocessor = MIMICPreprocessor(
        input_path=config["pipeline_config"]["input_path"], output_path=config["pipeline_config"]["output_path"]
    )
