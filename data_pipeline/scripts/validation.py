...
class MIMICDataValidator:
    ...
    def load_data(self, filename: str = "processed_discharge_summaries.csv") -> pd.DataFrame:
        ...
        if not os.path.exists(filepath):
            ...
            filepath = os.path.join("data_pipeline/data/raw", "mimic_discharge_labs.csv")
    ...
    def validate_completeness(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in df.columns:
            ...
            completeness_report["missing_percentage_per_column"][col] = round((missing_count / len(df)) * 100, 2)
        ...
        if "cleaned_text" in df.columns:
            completeness_report["records_without_text"] = int((df["cleaned_text"].isna() | (df["cleaned_text"] == "")).sum())
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        quality_report = {"text_length_issues": {}, "duplicate_records": {}, "outliers": {}, "data_consistency_issues": []}
    ...
    def validate_demographics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        demo_report = {"age_issues": {}, "ethnicity_distribution": {}, "gender_distribution": {}, "age_group_distribution": {}}
    ...
    def validate_cross_field_logic(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        cross_field_report = {"logical_inconsistencies": [], "inconsistency_count": 0}
        ...
        if "follow_up" in df.columns and "discharge_diagnosis" in df.columns:
            has_followup_no_dx = (
                (df["follow_up"].str.len() > 10) & ((df["discharge_diagnosis"].isna()) | (df["discharge_diagnosis"] == ""))
            ).sum()
        ...
        if "age_at_admission" in df.columns and "insurance" in df.columns:
            pediatric_medicare = (
                (df["age_at_admission"] < 18) & (df["insurance"].str.upper().str.contains("MEDICARE", na=False))
            ).sum()
    ...
    def validate_lab_values(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        lab_report = {"total_records_with_labs": 0, "invalid_lab_values": {}, "lab_value_statistics": {}}
    ...
    def validate_section_extraction(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        section_report = {"section_extraction_rates": {}, "average_section_lengths": {}, "empty_sections": {}}
    ...
    def run_validation_pipeline(self) -> Tuple[Dict, pd.DataFrame]:
        ...
        validation_report = {
            ...
            "dataset_info": {"total_records": int(len(df)), "total_columns": int(len(df.columns))},
        }
