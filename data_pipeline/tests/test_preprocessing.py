...
def preprocessor():
    ...
    return MIMICPreprocessor(input_path="tests/data", output_path="tests/output")
...
def test_all_duplicates(preprocessor):
    ...
    duplicate_data = pd.DataFrame(
        {"hadm_id": [1001, 1001, 1001], "subject_id": [101, 101, 101], "cleaned_text": ["same", "same", "same"]}
    )
...
def test_missing_demographic_columns(preprocessor):
    ...
    minimal_data = pd.DataFrame({"hadm_id": [1001, 1002], "subject_id": [101, 102], "cleaned_text": ["text1", "text2"]})
...
def test_abbreviations_case_insensitive(preprocessor):
    ...
    # Test various cases
    test_cases = ["Pt admitted", "PT admitted", "pt admitted"]
