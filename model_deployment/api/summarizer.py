...
class MedicalSummarizer:
    ...
    def __init__(self, use_gpu: bool = True, model_source: str = "huggingface"):
        ...
        try:
            ...
            self.model = AutoModelForSeq2SeqLM.from_pretrained(self.model_name, token=hf_token)
    ...
    def smart_extract(self, text: str, max_tokens: int = 1024) -> str:
        ...
        if sections["diagnosis"]:
            parts.append(f"Diagnosis: {sections['diagnosis']}")
        if sections["disposition"]:
            parts.append(f"Disposition: {sections['disposition']}")
        if sections["procedures"]:
            parts.append(f"Procedures: {sections['procedures']}")
        if sections["course"]:
            parts.append(f"Course: {sections['course']}")
    ...
    def generate_summary(self, text: str) -> Dict[str, str]:
        ...
        with torch.no_grad():
            summary_ids = self.model.generate(
                inputs.input_ids, max_length=150, min_length=40, num_beams=4, length_penalty=2.0, early_stopping=True
            )
