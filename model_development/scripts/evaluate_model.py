...
"""
    ...
    def calculate_bertscore(self, references: List[str], predictions: List[str]) -> Dict:
        ...
        P, R, F1 = bert_score(predictions, references, lang="en", model_type="microsoft/deberta-base-mnli", verbose=False)

        bertscore_results = {"precision": float(P.mean()), "recall": float(R.mean()), "f1": float(F1.mean())}
    ...
    def create_visualizations(self):
        ...
        axes[0, 0].hist(
            [rouge1_scores, rouge2_scores, rougeL_scores], label=["ROUGE-1", "ROUGE-2", "ROUGE-L"], alpha=0.7, bins=20
        )
        ...
        axes[0, 1].axvline(
            np.mean(inference_times), color="red", linestyle="--", label=f"Mean: {np.mean(inference_times):.2f}s"
        )
...
"""
    ...
    def run_complete_evaluation(self, sample_size: int = None, include_bertscore: bool = False) -> Dict:
        """
...
def main():
    ...
    parser.add_argument("--sample-size", type=int, default=None, help="Evaluate on sample (default: all)")
    parser.add_argument("--include-bertscore", action="store_true", help="Calculate BERTScore (slower)")

    args = parser.parse_args()

    # Run evaluation
    evaluator = ModelEvaluator(model_path=args.model_path, test_data_path=args.test_data, output_dir=args.output_dir)

    results = evaluator.run_complete_evaluation(sample_size=args.sample_size, include_bertscore=args.include_bertscore)
