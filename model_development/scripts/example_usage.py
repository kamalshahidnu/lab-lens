...
from src.training import GeminiInference, GeminiTrainer, load_gemini_model
...
def example_setup():
    ...
    trainer = GeminiTrainer(
        config_path="configs/gemini_config.json", model_name="gemini-2.0-flash-exp", output_dir="models/gemini"  # Latest model
    )

    # Process data with Gemini
    results = trainer.process_data(data_path="data_pipeline/data/processed/processed_discharge_summaries.csv")
...
def example_inference():
  ...
  """
    ...
    inferencer = GeminiInference(api_key=os.getenv("GOOGLE_API_KEY"), model_name="gemini-2.0-flash-exp")  # Latest model
...
def example_batch_processing():
    ...
    df_summarized = inferencer.process_dataframe(
        df_sample, input_column="cleaned_text", output_column="gemini_summary", max_length=150
    )
...
def example_load_model():
    ...
    # Load model
    model = load_gemini_model(api_key=os.getenv("GOOGLE_API_KEY"), model_name="gemini-2.0-flash-exp")  # Latest model
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--example", type=str, choices=["setup", "inference", "batch", "load"], default="load", help="Which example to run"
    )
