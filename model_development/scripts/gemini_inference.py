...
class GeminiInference:
    ...
    def __init__(self, api_key: Optional[str] = None, model_name: str = "gemini-2.0-flash-exp"):  # Latest model
        """
...
def main():
    ...
    parser.add_argument("--input", type=str, required=True, help="Input: text string or path to CSV file")
    parser.add_argument("--output", type=str, help="Output path for CSV (if input is CSV)")
    parser.add_argument("--input-column", type=str, default="cleaned_text", help="Input column name (for CSV)")
    parser.add_argument("--output-column", type=str, default="gemini_summary", help="Output column name (for CSV)")
    ...
    try:
        # Initialize inference model
        inferencer = GeminiInference(api_key=args.api_key, model_name=args.model_name)
        ...
        if os.path.exists(args.input):
            ...
            df = inferencer.process_dataframe(
                df, input_column=args.input_column, output_column=args.output_column, max_length=args.max_length
            )
