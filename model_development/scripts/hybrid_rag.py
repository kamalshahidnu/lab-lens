...
class HybridMedicalSummarizer:
    ...
    def build_simplification_kb(self):
        ...
        self.medical_kb = [
            ...
            {
                "medical": "congestive heart failure CHF",
                "simple": "weak heart pumping",
                "context": "heart can't pump enough blood",
            },
            {
                "medical": "chronic obstructive pulmonary disease COPD",
                "simple": "breathing disease",
                "context": "damaged lungs make breathing hard",
            },
            ...
            {
                "medical": "acute kidney injury AKI renal failure",
                "simple": "kidney failure",
                "context": "kidneys stop filtering blood",
            },
            ...
            {
                "medical": "brainstem hemorrhage",
                "simple": "bleeding in brain stem",
                "context": "bleeding in brain's control center",
            },
            {
                "medical": "gastrointestinal bleed GI bleed",
                "simple": "stomach or intestine bleeding",
                "context": "bleeding in digestive system",
            },
            {
                "medical": "subdural hematoma",
                "simple": "blood collection near brain",
                "context": "blood between brain and skull",
            },
            # Procedures
            {
                "medical": "coronary artery bypass graft CABG",
                "simple": "heart bypass surgery",
                "context": "reroute blood around blocked arteries",
            },
            {
                "medical": "percutaneous coronary intervention PCI",
                "simple": "opening heart arteries",
                "context": "balloon and stent to open blockage",
            },
            ...
            {
                "medical": "percutaneous endoscopic gastrostomy PEG",
                "simple": "feeding tube in stomach",
                "context": "tube through skin for feeding",
            },
            {
                "medical": "dialysis hemodialysis",
                "simple": "blood cleaning machine",
                "context": "machine filters blood for failed kidneys",
            },
            # Symptoms/Signs
    ...
    def create_retrieval_index(self):
        ...
        self.retriever = NearestNeighbors(n_neighbors=5, metric="cosine", algorithm="brute")  # For small dataset
    ...
    def extract_key_sections(self, discharge_text: str) -> Dict[str, str]:
        ...
        sections = {"diagnosis": "", "procedures": "", "disposition": "", "course": "", "medications": ""}
    ...
    def extract_from_unstructured(self, text: str, max_tokens: int = 1024) -> str:
        ...
        low_priority_keywords = ["stable", "improved", "history", "presented", "noted", "showed", "revealed", "consistent"]
    ...
    def summarize_with_bart(self, text: str, max_length: int = 150) -> str:
        ...
        # Tokenize
        inputs = self.bart_tokenizer(input_text, max_length=1024, truncation=True, return_tensors="pt", padding=True).to(
            self.device
        )
    ...
    def summarize_with_biobart(self, text: str, max_length: int = 150) -> str:
        ...
        # Tokenize
        inputs = self.biobart_tokenizer(prompted_text, max_length=1024, truncation=True, return_tensors="pt", padding=True).to(
            self.device
        )
        ...
        # Decode
        summary = self.biobart_tokenizer.decode(summary_ids[0], skip_special_tokens=True)
    ...
    def retrieve_simplifications(self, text: str, k: int = 5) -> List[Dict]:
        ...
        distances, indices = self.retriever.kneighbors(
            query_embedding.reshape(1, -1), n_neighbors=min(k, len(self.medical_kb))
        )
    ...
    def simplify_medical_terms(self, text: str, simplifications: List[Dict]) -> str:
        ...
        sorted_terms = sorted(simplifications, key=lambda x: len(x["medical"]), reverse=True)
    ...
    **FINAL OUTPUT MUST FOLLOW THE REQUIRED STRUCTURE.**
    ...
    def create_patient_summary(self, sections: Dict, biobart_summary: str, simplifications: List[Dict]) -> str:
    ...
    def create_patient_summary_fallback(self, sections: Dict, biobart_summary: str, simplifications: List[Dict]) -> str:
    ...
    def generate_summary(self, discharge_text: str) -> Dict[str, str]:
        ...
        patient_summary_biobart = self.create_patient_summary(sections, biobart_summary, simplifications)

        # Step 4b: Create patient summary using BART
        print("4b. Creating patient summary (BART → Gemini)...")
        patient_summary_bart = self.create_patient_summary(sections, bart_summary, simplifications)
