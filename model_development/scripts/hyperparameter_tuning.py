...
class HyperparameterTuner:
    ...
    def objective(
        ...
        logger.info(
            f"Trial {trial.number}: temperature={temperature}, "
        ...
        try:
            ...
            model = GeminiSummarizer(
                api_key=self.api_key, model_name=self.model_name, temperature=temperature, max_output_tokens=max_output_tokens
            )
    ...
    def optimize(
        ...
        study = optuna.create_study(
            study_name=self.study_name, direction=direction, sampler=optuna.samplers.TPESampler(seed=42)
        )

        # Create objective function with data
        objective_func = lambda trial: self.objective(trial, train_data, val_data, input_column, reference_column, sample_size)
