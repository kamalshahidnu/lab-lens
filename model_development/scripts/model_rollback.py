...
class ModelRollback:
    ...
    def __init__(self, tracking_uri: Optional[str] = None, experiment_name: str = "gemini-medical-summarization"):
        """
    ...
    def get_latest_models(self, n: int = 2) -> List[Dict]:
        ...
        try:
            ...
            # Get all runs
            runs = mlflow.search_runs(experiment_ids=[experiment.experiment_id], order_by=["start_time DESC"], max_results=n)
    ...
    def compare_models(
        ...
        if previous_metrics.get(primary_metric, 0.0) > 0:
            improvement = (
                (current_metrics.get(primary_metric, 0.0) - previous_metrics.get(primary_metric, 0.0))
            ...
            if improvement < -threshold * 100:  # More than threshold% worse
                ...
                logger.warning(f"Model performance degraded: {improvement:.2f}% worse. " f"Rollback recommended.")
        ...
        for metric_name in set(list(current_metrics.keys()) + list(previous_metrics.keys())):
            ...
            all_metrics[metric_name] = {"current": current_val, "previous": previous_val, "change": current_val - previous_val}
    ...
    def check_and_rollback(self, current_run_id: str, primary_metric: str = "rougeL_f", threshold: float = 0.05) -> Dict:
        """
        ...
        if len(models) < 2:
            ...
            return {"rollback_needed": False, "reason": "insufficient_models", "models_compared": len(models)}
        ...
        should_rollback, comparison = self.compare_models(
            current_metrics, previous_metrics, primary_metric=primary_metric, threshold=threshold
        )
...
def main():
    ...
    parser.add_argument("--run-id", type=str, required=True, help="Current model run ID to check")
    parser.add_argument("--primary-metric", type=str, default="rougeL_f", help="Primary metric for comparison")
    parser.add_argument("--threshold", type=float, default=0.05, help="Performance degradation threshold")

    args = parser.parse_args()

    rollback = ModelRollback()
    result = rollback.check_and_rollback(
        current_run_id=args.run_id, primary_metric=args.primary_metric, threshold=args.threshold
    )
