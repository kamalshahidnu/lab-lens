...
class ModelValidator:
    ...
    def calculate_bleu(self, predictions: List[str], references: List[str]) -> Dict[str, float]:
        ...
        if SACREBLEU_AVAILABLE and bleu_scores:
            try:
                ...
                sacrebleu_score = self.sacrebleu.corpus_score(predictions, [[ref] for ref in references])
    ...
    def validate_model(
        self, predictions: List[str], references: List[str], include_rouge: bool = True, include_bleu: bool = True
    ) -> Dict[str, float]:
        """
    ...
    def validate_from_dataframe(
        ...
        valid_pairs = [
            (p, r) for p, r in zip(predictions, references) if pd.notna(p) and pd.notna(r) and p.strip() and r.strip()
        ]
        ...
        return self.validate_model(
            list(predictions_clean), list(references_clean), include_rouge=include_rouge, include_bleu=include_bleu
        )
