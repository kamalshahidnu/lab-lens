...
def prepare_summarization_dataset(input_file: str, output_dir: str, use_gemini: bool = True) -> Dict[str, str]:
    ...
    for idx in range(len(df_valid)):
        if idx % 50 == 0:
            print(
                f" Progress: {idx}/{len(df_valid)} ({idx/len(df_valid)*100:.0f}%) | Success: {api_success} | Fallback: {api_errors}"
            )
    ...
    train_val, test_df = train_test_split(
        df_valid, test_size=0.15, random_state=42, stratify=df_valid["age_group"] if "age_group" in df_valid.columns else None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, default="data_pipeline/data/processed/processed_discharge_summaries.csv", help="Input CSV file"
    )
    parser.add_argument("--output", type=str, default="model-development/data/model_ready", help="Output directory")
    parser.add_argument(
        "--use-gemini", action="store_true", help="Use Gemini API for narratives (requires GOOGLE_API_KEY env var)"
    )
    parser.add_argument("--no-gemini", action="store_true", help="Force rule-based narratives (no API)")
