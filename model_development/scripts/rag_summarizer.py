...
class MedicalRAGSummarizer:
    ...
    def load_llama_model(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            base_model, torch_dtype=torch.float16, device_map="cpu", low_cpu_mem_usage=True, trust_remote_code=True
        )
    ...
    def build_knowledge_base(self):
        ...
        self.knowledge_base = [
            ...
            {"medical": "bilateral", "simple": "on both sides", "context": "affects both left and right"},
            {"medical": "acute", "simple": "sudden or severe", "context": "happened quickly"},
            {"medical": "chronic", "simple": "long-term ongoing", "context": "been happening for a while"},
            {"medical": "exacerbation", "simple": "got worse", "context": "condition flared up"},
            {"medical": "edema", "simple": "swelling from fluid", "context": "body holding extra water"},
            {"medical": "hypoxia", "simple": "not enough oxygen", "context": "low oxygen levels"},
            {"medical": "tachycardia", "simple": "fast heart rate", "context": "heart beating too quickly"},
            {"medical": "bradycardia", "simple": "slow heart rate", "context": "heart beating too slowly"},
            {"medical": "hypertension", "simple": "high blood pressure", "context": "needs medication to control"},
            {"medical": "hypotension", "simple": "low blood pressure", "context": "may cause dizziness"},
        ]
    ...
    def extract_key_information(self, discharge_text: str) -> Dict:
        ...
        info = {"diagnosis": "", "procedures": [], "disposition": "", "medications": []}
    ...
    def generate_rag_summary(self, discharge_text: str) -> str:
        ...
        relevant_context = self.retrieve_relevant_simplifications(
            discharge_text[:1000], k=5  # Use first 1000 chars for retrieval
        )

        # 3. Build context for generation
        context_text = "\n".join([f"- {entry['medical']} means {entry['simple']}" for entry in relevant_context])
...
Based on the medical definitions provided, here's what happened: """
    ...
    def evaluate_with_rag(self, test_df: pd.DataFrame, sample_size: int = 10):
        ...
        for idx in range(min(sample_size, len(test_df))):
            ...
            scores.append({"hadm_id": row.get("hadm_id", idx), "rag_summary": rag_summary})
...
if __name__ == "__main__":
    ...
    rag_summarizer = MedicalRAGSummarizer(model_path="../saved_models/content/saved_model")
