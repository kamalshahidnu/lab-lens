...
import numpy as np
import torch
from validate_model import format_prompt, load_model
...
"""
    ...
    with torch.no_grad():
        baseline_outputs = model.generate(
            baseline_inputs.input_ids, max_new_tokens=100, temperature=0.7, do_sample=True, pad_token_id=tokenizer.eos_token_id
        )
    ...
    for section_name, keywords in sections.items():
        ...
        modified_text_output = tokenizer.decode(
            modified_outputs[0][len(modified_inputs.input_ids[0]) :], skip_special_tokens=True
        )
