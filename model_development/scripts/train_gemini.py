...
from src.utils.error_handling import ErrorContext, ErrorHandler, safe_execute
...
"""
    ...
    def setup_model(self) -> GeminiSummarizer:
        ...
        model = load_gemini_model(api_key=self.api_key, model_name=self.model_name)
    ...
    def process_data(self, data_path: str, output_path: Optional[str] = None) -> Dict:
        """
...
def main():
    ...
    parser.add_argument("--data-path", type=str, required=True, help="Path to processed data CSV file")
    ...
    parser.add_argument(
        "--output-dir", type=str, default="models/gemini", help="Output directory for configuration and results"
    )
    ...
    try:
        trainer = GeminiTrainer(
            config_path=args.config, api_key=args.api_key, model_name=args.model_name, output_dir=args.output_dir
        )

        results = trainer.process_data(data_path=args.data_path, output_path=args.output)
