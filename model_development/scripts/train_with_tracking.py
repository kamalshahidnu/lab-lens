...
class CompleteModelTrainer:
    ...
    def __init__(
        ...
        self.trainer = GeminiTrainer(
            config_path=config_path, api_key=self.api_key, model_name=self.model_name, output_dir=str(self.output_dir)
        )
    ...
    def load_data_from_pipeline(self, data_path: str, train_split: float = 0.8, val_split: float = 0.1) -> tuple:
        """
    ...
    def train_and_evaluate(
        self, data_path: str, hyperparameters: Optional[Dict] = None, run_name: Optional[str] = None
    ) -> Dict:
        """
        ...
        with MLflowTracker(run_name=run_name) as tracker:
            ...
            if self.enable_hyperparameter_tuning and hyperparameters is None:
                ...
                tuner = HyperparameterTuner(
                    api_key=self.api_key, model_name=self.model_name, n_trials=10  # Reduced for faster execution
                )
                ...
                if self.enable_sensitivity_analysis:
                    sensitivity_results = self.sensitivity_analyzer.analyze_hyperparameter_sensitivity(optimization_history)
                    ...
                    plot_paths = self.sensitivity_analyzer.create_sensitivity_plots(
                        optimization_history, str(self.output_dir / "sensitivity_plots")
                    )
            ...
            if best_hyperparams is None:
                best_hyperparams = {"temperature": 0.3, "max_output_tokens": 2048, "max_length": 150}
            ...
            for idx, row in val_sample.iterrows():
                ...
                try:
                    summary = model.summarize(input_text, max_length=best_hyperparams.get("max_length", 150))
            ...
            validation_metrics = self.validator.validate_model(val_predictions, val_references)
            ...
            model_selection_result = self._select_best_model_after_bias_check(
                metrics=validation_metrics, bias_report=bias_report, run_id=tracker.run_id
            )
            ...
            tracker.log_params({"selection_reason": model_selection_result.get("reason", "")})
            ...
            if tracker.run_id and model_selection_result.get("selected", True):
                try:
                    rollback_result = self.rollback.check_and_rollback(
                        current_run_id=tracker.run_id, primary_metric="rougeL_f", threshold=0.05
                    )
    ...
    def _select_best_model_after_bias_check(self, metrics: Dict, bias_report: Optional[Dict], run_id: Optional[str]) -> Dict:
        """
        ...
        if bias_report:
            ...
            selection_result["reason"] = (
                f'Selected based on combined score: validation={metrics.get("rougeL_f", 0.0):.3f}, bias={bias_score:.3f}'
            )
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Complete model training with MLOps components")
    parser.add_argument("--data-path", type=str, required=True, help="Path to processed data from pipeline")
    parser.add_argument("--config", type=str, help="Path to configuration file")
    parser.add_argument("--api-key", type=str, help="Google AI API key (or set GOOGLE_API_KEY env var)")
    parser.add_argument("--model-name", type=str, default="gemini-2.0-flash-exp", help="Gemini model identifier")
    parser.add_argument("--output-dir", type=str, default="models/gemini", help="Output directory")
    parser.add_argument("--enable-tuning", action="store_true", help="Enable hyperparameter tuning")
    parser.add_argument("--disable-bias-detection", action="store_true", help="Disable bias detection")
    parser.add_argument("--disable-sensitivity", action="store_true", help="Disable sensitivity analysis")
    parser.add_argument("--run-name", type=str, help="Name for MLflow run")
    ...
    try:
        ...
        results = trainer.train_and_evaluate(data_path=args.data_path, run_name=args.run_name)
