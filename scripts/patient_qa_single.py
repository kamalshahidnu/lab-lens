...
def interactive_single_patient(hadm_id: int, data_path: str):
    ...
    try:
        qa_system = PatientQA(data_path=data_path, hadm_id=hadm_id)  # Single-patient mode
...
Examples:
 ...
 python scripts/patient_qa_single.py --hadm-id 130656 --question "What are my diagnoses?"
    ...
    parser.add_argument("--hadm-id", type=int, required=True, help="HADM ID of the patient (required)")
    ...
    parser.add_argument("--view", action="store_true", help="View patient record summary before Q&A")

    parser.add_argument("--question", type=str, help="Single question to answer (if not provided, enters interactive mode)")

    parser.add_argument(
        "--embedding-model", type=str, default="all-MiniLM-L6-v2", help="Sentence transformer model for embeddings"
    )

    parser.add_argument("--gemini-model", type=str, default="gemini-2.0-flash-exp", help="Gemini model for answer generation")
