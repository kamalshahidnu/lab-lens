...
def test_risk_prediction(
  ...
  try:
    ...
    result['recommendations'] = [prediction_details.get('recommendation', '')] if prediction_details.get('recommendation') else []
  ...
  try:
    ...
    if result['has_disease']:
      ...
      if result['diseases_detected']:
        diseases_str = ', '.join(str(d) for d in result['diseases_detected']) if isinstance(result['diseases_detected'], list) else str(result['diseases_detected'])
...
Examples:
 ...
 python scripts/test_all_models.py --image-path xray.jpg --age 65 --gender M
  ...
  parser.add_argument('--interactive', action='store_true',
            help='Interactive mode: prompts for image path if not provided')
