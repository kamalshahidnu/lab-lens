...
class CompleteModelTester:
    ...
    def test_discharge_summary(self, text: str = SAMPLE_DISCHARGE_SUMMARY) -> Dict:
        ...
        result = {"status": "failed", "input_length": len(text), "summary": None, "error": None}
    ...
    def test_risk_prediction(
        self, discharge_text: str = SAMPLE_DISCHARGE_SUMMARY, patient_info: Optional[Dict] = None
    ) -> Dict:
        """
    ...
    def validate_model_performance(
        self, predictions: list, references: list, include_rouge: bool = True, include_bleu: bool = True
    ) -> Dict:
        """
        ...
        try:
            metrics = self.validator.validate_model(
                predictions=predictions, references=references, include_rouge=include_rouge, include_bleu=include_bleu
            )
    ...
    def run_all_tests(self, discharge_text: str = SAMPLE_DISCHARGE_SUMMARY, patient_info: Optional[Dict] = None) -> Dict:
        """
        Run all tests
    ...
    def run_batch_tests(self, records: list) -> Dict:
        ...
        for i, record_data in enumerate(records, 1):
            ...
            record_result = {
                ...
                "overall_status": (
                    "success" if (summary_result["status"] == "success" and risk_result["status"] == "success") else "partial"
                ),
            }
        ...
        if self.enable_validation and self.validator:
            ...
            if len(predictions) > 0 and len(predictions) == len(references):
                validation_metrics = self.validate_model_performance(predictions=predictions, references=references)
        ...
        print(
            f" Pass Rate: {100 * batch_results['summary']['discharge_summary_success'] / batch_results['summary']['total_tested']:.1f}%"
        )
        print(f"\nRisk Prediction Tests:")
        print(f" Passed: {batch_results['summary']['risk_prediction_success']}/{batch_results['summary']['total_tested']}")
        print(
            f" Pass Rate: {100 * batch_results['summary']['risk_prediction_success'] / batch_results['summary']['total_tested']:.1f}%"
        )
    ...
    try:
        ...
        if num_records is not None and num_records > 0:
            ...
            for idx, record in records_df.iterrows():
                ...
                records.append({"discharge_text": discharge_text, "patient_info": patient_info})

                print(
                    f"  Record {len(records)}: HADM ID {patient_info['hadm_id']}, "
        ...
        # Add optional fields if available
        for field in ["ethnicity", "insurance", "admission_type", "length_of_stay", "complexity_score", "urgency_indicator"]:
            if field in record and pd.notna(record[field]):
                patient_info[field] = record[field]
        ...
        return {"discharge_text": discharge_text, "patient_info": patient_info}
...
Examples:
 ...
 python scripts/test_complete_model.py --index 0 --age 70 --gender F
    ...
    parser.add_argument(
        "--index", type=int, default=None, help="Row index (0-based) to select from MIMIC data (single record)"
    )

    parser.add_argument(
        "--hadm-id", type=int, default=None, help="Hospital admission ID (hadm_id) to select from MIMIC data (single record)"
    )

    parser.add_argument("--use-sample", action="store_true", help="Use hardcoded sample data instead of MIMIC data")

    parser.add_argument("--age", type=int, default=None, help="Override patient age for risk prediction")

    parser.add_argument("--gender", type=str, default=None, help="Override patient gender (M/F) for risk prediction")

    parser.add_argument("--symptoms", type=str, default=None, help="Patient symptoms (not used if loading from MIMIC)")

    parser.add_argument("--output", type=str, default="test_results.json", help="Output file for test results")

    parser.add_argument("--validate", action="store_true", help="Enable model validation with ROUGE/BLEU metrics")

    parser.add_argument("--train", action="store_true", help="Train/develop model before testing (requires data path)")

    parser.add_argument(
        "--train-config", type=str, default=None, help="Path to training configuration file (for --train mode)"
    )

    parser.add_argument(
        "--model-dir", type=str, default="models/gemini", help="Directory to save trained models (for --train mode)"
    )
    ...
    if not args.use_sample:
        ...
        mimic_data = load_mimic_data(
            data_path=args.data_path, index=args.index, hadm_id=args.hadm_id, num_records=args.num_records
        )
        ...
        if args.use_sample:
            ...
            try:
                ...
                train_df, val_df, test_df = trainer.load_data_from_pipeline(
                    data_path=args.data_path, train_split=0.8, val_split=0.1
                )
    ...
    try:
        ...
        if batch_records is not None:
            ...
            success_rate = (
                results["summary"]["successful"] / results["summary"]["total_tested"]
            ...
            results = tester.run_all_tests(discharge_text=discharge_text, patient_info=patient_info)
            ...
            if args.validate and tester.validator:
                ...
                if summary_result.get("status") == "success" and summary_result.get("summary"):
                    validation_metrics = tester.validate_model_performance(
                        predictions=[summary_result["summary"]], references=[discharge_text]
                    )
